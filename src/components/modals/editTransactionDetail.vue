      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="editTransactionLabel">Edit Item</h5>
            <button
              type="button"
              class="btn-close"
...
import { updateTransactionItemDetail } from '@/services/transactionStockService'
...
export default {
  ...
  methods: {
    ...
    fillForm() {
      this.item = this.itemProps.Item
      this.quantity = this.itemProps.Quantity
      this.itemUnit = this.itemProps.ItemUnit
      this.note = this.itemProps.Note
    },

    handleFormSubmit() {
      const payload = {
        index: null,
        Oid: this.itemProps.Oid,
        Item: this.item,
      ...
      updateTransactionItemDetail(this.$route.params.Oid, payload).then((response) => {
        if (response.status === 200) {
